...
void First::moveToEnd(std::string & text, int &pos, int &L, int x)
{
	move(text, x);
	x = int(text.size()) - 4;
	int y = 4 - (x - pos) % 4;
...
void First::loopNearEnd(std::string & text, int &pos, int &L, int x)
{
	while ((x - pos) % 4 != 0)
	{
...
std::string First::findPrefix(const std::string & text, int L)
{
...
std::string First::brutal(std::string & text, int L)
{
